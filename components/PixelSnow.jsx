  ...
  for (int i = 0; i < 128; i++) {
    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        float flakeSize = max(0.01, 1.25 * depth * halfInvResX);
        ...
        if (dist < flakeSize) {
          float flakeSizeRatio = 0.01 / flakeSize;
...
export default function PixelSnow({
  ...
  const handleResize = useCallback(() => {
    ...
    resizeTimeoutRef.current = window.setTimeout(() => {
      ...
      // Calculate target render size based on pixelResolution
      const renderW = Math.round(pixelResolution);
      const renderH = Math.round(pixelResolution / aspect);
      
      // Resize renderer buffer, but don't touch style (handled by CSS)
  ...
  useEffect(() => {
    ...
    renderer.setPixelRatio(1); // Force 1:1 pixel mapping for the low-res buffer
    ...
    const material = new ShaderMaterial({
      ...
      uniforms: {
        uTime: { value: 0 },
        uResolution: { value: new Vector2(renderW, renderH) },
        uSpeed: { value: speed },
  ...
  useEffect(() => {
    ...
    // If pixelResolution changed, we might need to resize the renderer

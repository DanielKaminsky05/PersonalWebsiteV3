            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                <div className="space-y-2">
                    {data.map((submission: TRecentSubmission, i: number) => (
                        <div 
                            key={i} 
                            className="flex items-center justify-between w-full px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors border border-white/5"
                        >
                            <div className="flex items-center gap-2 min-w-0 flex-1">
                                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${submission.statusDisplay === "Accepted" ? "bg-green-500" : "bg-red-500"}`} />
                                <p className="text-sm text-gray-200 truncate">{submission.title}</p>
                            </div>
